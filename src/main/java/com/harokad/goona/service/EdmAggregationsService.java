...
import com.harokad.goona.domain.EdmAggregationItem;
...
public class EdmAggregationsService {
    ...
    /*## top ten used word
    # exlude too generic words
    @Value("${edm.top_terms.exlusion_regex}")*/
    private String edmTopTermsExclusionRegex = "[a-z]{1,2}|[dlmcs]es|data|docs|documents|edm|files|simple|page";
    ...
    public List<EdmAggregationItem> getTopTerms(String relativeWordSearch) {
        ...
        TermsBuilder aggregationBuilder;
        if(filesExtensions.isEmpty()){
        	aggregationBuilder = AggregationBuilders.terms("agg_nodePath").field("nodePath.nodePath_simple").exclude(edmTopTermsExclusionRegex).size(10);	
        } else {
        	String filesExtension = StringUtils.join(filesExtensions, "|");
        	aggregationBuilder = AggregationBuilders.terms("agg_nodePath").field("nodePath.nodePath_simple").exclude(edmTopTermsExclusionRegex + "|" + filesExtension).size(10);	
        }
